...
- ссылка и QR-код являются секретными данными доступа
...
Используйте корректные платёжные и контактные данные, которые Apple принимает для выбранного региона.
Не используйте чужой адрес, чужой телефон или выдуманные персональные данные.
...
Примеры формата, если выбран способ оплаты `None` (`No Payment Method`):
...
Английская версия: [`docs/clients.md`](clients.md).
