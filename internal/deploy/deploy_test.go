...
func TestRenderBundleWithOverride(t *testing.T) {
	...
	for _, want := range []string{
		...
		"expr: (1 - (node_memory_MemAvailable_bytes{job=\"node_exporter\"} / node_memory_MemTotal_bytes{job=\"node_exporter\"})) * 100 > 92",
		"for: 20m",
		"description: \"Memory usage is above 92% for 20 minutes.\"",
		...
		"expr: node_memory_MemAvailable_bytes{job=\"node_exporter\"} < 157286400",
		"description: \"MemAvailable is below 150 MiB for 20 minutes.\"",
