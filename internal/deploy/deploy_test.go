...
func TestRenderBundleWithOverride(t *testing.T) {
	...
	for _, want := range []string{"--data.retention=168h", "/run/udev:/run/udev:ro", "/dev/kmsg:/dev/kmsg:ro", "--collector.textfile.directory=/var/lib/node-exporter/textfile", "/var/lib/ovpn-node-exporter-textfile:/var/lib/node-exporter/textfile:ro"} {
