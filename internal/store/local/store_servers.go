...
func (s *Store) AddServer(ctx context.Context, srv *model.Server) error {
	...
	res, err := s.db.ExecContext(ctx, `
		INSERT INTO servers (
			...
			reality_short_ids, reality_server_name, reality_target, proxy_preset, proxy_service_uuid,
			primary_profile, enabled_profiles, enabled, created_at, updated_at
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, srv.Name, srv.Role, srv.Host, srv.Domain, srv.SSHUser, srv.SSHPort, srv.SSHIdentityFile, srv.SSHKnownHostsFile,
		...
		srv.RealityShortIDs, srv.RealityServerName, srv.RealityTarget, srv.ProxyPreset, srv.ProxyServiceUUID,
		srv.PrimaryProfile, srv.EnabledProfiles, boolToInt(srv.Enabled), now, now)
	if err != nil {
		return err
	}
...
func (s *Store) UpdateServer(ctx context.Context, srv *model.Server) error {
	...
	_, err = s.db.ExecContext(ctx, `
		UPDATE servers SET
			...
			ssh_strict_host_key=?, xray_version=?, reality_private_key=?, reality_public_key=?,
			reality_short_ids=?, reality_server_name=?, reality_target=?, proxy_preset=?, proxy_service_uuid=?,
			primary_profile=?, enabled_profiles=?, enabled=?, updated_at=?
		WHERE id=?
	`, srv.Role, srv.Host, srv.Domain, srv.SSHUser, srv.SSHPort, srv.SSHIdentityFile, srv.SSHKnownHostsFile,
		boolToInt(srv.SSHStrictHostKey), srv.XrayVersion, realityPrivateKey, srv.RealityPublicKey,
		srv.RealityShortIDs, srv.RealityServerName, srv.RealityTarget, srv.ProxyPreset, srv.ProxyServiceUUID,
		srv.PrimaryProfile, srv.EnabledProfiles, boolToInt(srv.Enabled), now, srv.ID)
	return err
}
...
func (s *Store) GetServerByName(ctx context.Context, name string) (*model.Server, error) {
	row := s.db.QueryRowContext(ctx, `
		SELECT id, name, role, host, domain, ssh_user, ssh_port, ssh_identity_file, ssh_known_hosts_file,
			...
			reality_server_name, reality_target, proxy_preset, proxy_service_uuid, primary_profile, enabled_profiles,
			enabled, created_at, updated_at, last_deploy_at
